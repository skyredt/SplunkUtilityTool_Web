require([
    ...
    var $reportSearchInput = $("#sutw-report-search");
    var $reportClearSearchButton = $("#sutw-report-clear-search");
    var $reportClearSelectionButton = $("#sutw-report-clear-selection");
    var $reportFilterSummary = $("#sutw-report-filter-summary");
    var $rangeOptions = $(".sutw-range-option");
    var $customRangeEarliest = $("#sutw-custom-range-earliest");
    var $customRangeLatest = $("#sutw-custom-range-latest");
    var $customRangeApplyButton = $("#sutw-custom-range-apply");
    var $customRangeFeedback = $("#sutw-custom-range-feedback");
    ...
    var $executionReadinessSection = $("#sutw-section-execution-readiness");
    var $phaseCapabilitiesSection = $("#sutw-section-phase-capabilities");
    var $transitionPolicySection = $("#sutw-section-transition-policy");
    var $actionIntentsSection = $("#sutw-section-action-intents");
    var $executionPlanSection = $("#sutw-section-execution-plan");
    var $executionRequestSection = $("#sutw-section-execution-request");
    ...
    var activeTimeRange = $.extend({}, defaultTimeRange);
    var timeTokenPattern = /^[A-Za-z0-9@._:+-]{1,64}$/;
    ...
    function getSelectedReports() {
        ...
        $.each(eligibleReports, function(_, report) {
            if (report && report.id) {
                reportMap[report.id] = report;
            }
        });

        return $.map(selectedReportIds, function(reportId) {
            return reportMap[reportId] || null;
        });
    }

    function getSelectedReportIds() {
        return selectedReportIds.slice();
    }

    function isReportSelected(reportId) {
        return $.inArray(reportId, selectedReportIds) !== -1;
    }

    function toggleSelectedReport(reportId) {
        var index = $.inArray(reportId, selectedReportIds);
    ...
    function getFilteredEligibleReports() {
        if (!reportFilterText) {
            return eligibleReports.slice();
        }

        return $.grep(eligibleReports, function(report) {
            var haystack = [
                report && report.title ? report.title : "",
                report && report.name ? report.name : "",
                report && report.app ? report.app : "",
                report && report.owner ? report.owner : "",
                report && report.description ? report.description : ""
            ].join(" ").toLowerCase();

            return haystack.indexOf(reportFilterText) !== -1;
        });
    }

    function updateReportFilterSummary(visibleCount, totalCount) {
        var summary = "Showing " + visibleCount + " of " + totalCount + " eligible reports.";

        if (reportFilterText) {
            summary += " Filter: " + reportFilterText + ".";
        }

        if (selectedReportIds.length > 0) {
            summary += " " + formatReportCount(selectedReportIds.length) + " selected.";
        }

        $reportFilterSummary.text(summary);
        $reportClearSearchButton.prop("disabled", !reportFilterText);
        $reportClearSelectionButton.prop("disabled", selectedReportIds.length === 0);
    }

    function renderEligibleReportList(emptyMessage) {
        var filteredReports;

        if (!Array.isArray(eligibleReports) || eligibleReports.length === 0) {
            $reportList.empty();
            updateReportFilterSummary(0, 0);
            updateDraftSelection();
            return;
        }

        filteredReports = getFilteredEligibleReports();

        if (filteredReports.length === 0) {
            $reportList.html("<li>" + escapeHtml(emptyMessage || "No eligible reports match the current filter.") + "</li>");
        } else {
            $reportList.html($.map(filteredReports, buildReportRow).join(""));
        }

        $reportFeedback.text("Loaded " + eligibleReports.length + " eligible reports from the backend.");
        updateReportFilterSummary(filteredReports.length, eligibleReports.length);
        updateDraftSelection();
    }

    function setCustomRangeFeedback(message, isError) {
        $customRangeFeedback.text(message);
        $customRangeFeedback.toggleClass("is-error", !!isError);
    }

    function setPresetTimeRange($option) {
        if (!$option || $option.length === 0) {
            activeTimeRange = $.extend({}, defaultTimeRange);
            $rangeOptions.removeClass("is-active");
            $customRangeApplyButton.removeClass("is-active");
            setCustomRangeFeedback("Using the default preset time range for preview and submission.", false);
            return;
        }

        activeTimeRange = {
            label: $option.data("rangeLabel"),
            earliest: $option.data("earliest"),
            latest: $option.data("latest")
        };
        $rangeOptions.removeClass("is-active");
        $option.addClass("is-active");
        $customRangeApplyButton.removeClass("is-active");
        setCustomRangeFeedback("Using preset range " + activeTimeRange.label + " for preview and submission.", false);
    }

    function applyCustomTimeRange() {
        var earliest = $.trim($customRangeEarliest.val() || "");
        var latest = $.trim($customRangeLatest.val() || "");

        if (!timeTokenPattern.test(earliest) || !timeTokenPattern.test(latest)) {
            setCustomRangeFeedback("Enter valid Splunk time tokens such as -2d@d, now, or 2026-03-09T00:00:00.", true);
            return false;
        }
        ...
        $rangeOptions.removeClass("is-active");
        $customRangeApplyButton.addClass("is-active");
        setCustomRangeFeedback("Using custom earliest/latest tokens for preview and submission.", false);
        return true;
    }

    function showAdvancedBatchSections(isVisible) {
        $executionReadinessSection.prop("hidden", !isVisible);
        $phaseCapabilitiesSection.prop("hidden", !isVisible);
        $transitionPolicySection.prop("hidden", !isVisible);
        $actionIntentsSection.prop("hidden", !isVisible);
        $executionPlanSection.prop("hidden", !isVisible);
        $executionRequestSection.prop("hidden", !isVisible);
    }

    getActiveTimeRange = function() {
        return $.extend({}, activeTimeRange);
    };

    buildReportRow = function(report) {
        var title = report.title || report.name || "Unnamed Report";
        var metaText = "App: " + (report.app || "unknown") + " | Owner: " + (report.owner || "unknown");
        var isSelected = isReportSelected(report.id);
        ...
        return [
            "<li>",
            '  <button class="sutw-report-row' + (isSelected ? ' is-selected' : '') + '" type="button" data-report-id="' + escapeHtml(report.id) + '" data-report-label="' + escapeHtml(title) + '">',
            '    <span class="sutw-report-row__main">',
            '      <span class="sutw-report-row__title">' + escapeHtml(title) + "</span>",
            '      <span class="sutw-report-row__meta">' + escapeHtml(metaText) + "</span>",
            "    </span>",
            '    <span class="sutw-report-row__state">' + escapeHtml(isSelected ? "Selected" : "Click to select") + "</span>",
            "  </button>",
            "</li>"
        ].join("");
    };

    renderReportLoadFailure = function(message) {
        eligibleReports = [];
        selectedReportIds = [];
        reportFilterText = "";
    ...
    renderEligibleReports = function(reports) {
        var eligibleIds = {};

        eligibleReports = Array.isArray(reports) ? reports.slice() : [];
        $.each(eligibleReports, function(_, report) {
            if (report && report.id) {
                eligibleIds[report.id] = true;
            }
        });
        selectedReportIds = $.grep(selectedReportIds, function(reportId) {
            return !!eligibleIds[reportId];
        });
        ...
        renderEligibleReportList();
    };

    updateDraftSelection = function(preserveSubmissionFeedback) {
        var selectedReports = $.map(getSelectedReports(), function(report) {
            return report.title || report.name || report.id || "Unnamed Report";
        });
        var selectedCount = selectedReportIds.length;
        var activeRange = getActiveTimeRange();
        var isAcknowledged = $reviewAcknowledgement.is(":checked");
        var isStartEnabled = selectedCount > 0 && isAcknowledged;
        var readinessText = "Select at least one report";
        ...
        if (selectedCount > 0) {
            previewLabels = selectedReports.slice(0, 3);
            reportSummary = "Selected " + formatReportCount(selectedCount) + ": " + previewLabels.join(", ");
            if (selectedCount > previewLabels.length) {
                reportSummary += ", +" + (selectedCount - previewLabels.length) + " more";
            }
            reportSummary += ". Time range: " + activeRange.label + " (" + activeRange.earliest + " to " + activeRange.latest + ").";
            readinessText = "Acknowledge review to enable tracked submission";
            actionNote = "Review the current selection and acknowledge it to enable the non-destructive tracked submission.";
        }
    ...
    var originalApplyBatchStatusResponse = applyBatchStatusResponse;
    applyBatchStatusResponse = function(response) {
        var batch = response && response.batch ? response.batch : null;
        var progress = batch && batch.progress ? batch.progress : null;
        var submission = batch && batch.submission ? batch.submission : null;
        ...
        if (!batch || !progress || !submission) {
            ...
            setProgressState(
                0,
                "Tracked status not ready",
                "The backend returned an incomplete tracked batch response. Reopen the batch or refresh recent batches to try again.",
                "Current state: Waiting for status"
            );
            setSummaryState(
                currentBatchId || "Unknown batch",
                "Waiting for status",
                ...
                "Tracked batch details are not ready yet. Reopen the batch or refresh recent batches to try again."
            );
            showAdvancedBatchSections(false);
            setBatchDetailMode(true);
            renderReportStatuses([], "Detailed per-report status is not available yet.");
            renderEventTimeline([], "Lifecycle events will appear after the backend returns a complete tracked batch record.");
            renderRecentBatches(recentBatches, defaultRecentBatchMessage);
            return;
        }

        showAdvancedBatchSections(true);
        originalApplyBatchStatusResponse(response);
    };

    renderBatchStatusFailure = function(batchId, message) {
        clearBatchStatusPoll();
        setProgressState(
            0,
            "Tracked status not available",
            message,
            "Current state: Waiting for status"
        );
        setSummaryState(
            ...
            "Unknown",
            "Waiting for response",
            message + " Reopen the batch or refresh recent batches to try again."
        );
        showAdvancedBatchSections(false);
        setBatchDetailMode(true);
        renderReportStatuses([], "Per-report status will appear after a successful status refresh.");
        renderEventTimeline([], "Lifecycle events will appear after a successful status refresh.");
        renderRecentBatches(recentBatches, defaultRecentBatchMessage);
        setShellStatus("Tracked batch status could not be loaded.");
    };

    var originalResetSummaryState = resetSummaryState;
    resetSummaryState = function() {
        originalResetSummaryState();
        showAdvancedBatchSections(false);
        setSummaryState(
            "No tracked batch yet",
            "Waiting for first submission",
            "0 reports",
            "No status requested yet",
            "No tracked batch yet. Submit a validated request to create a tracked batch ID."
        );
    };

    $reportList.off("click", ".sutw-report-row").on("click", ".sutw-report-row", function() {
        ...
        clearTrackedBatch();
        toggleSelectedReport(reportId);
        renderEligibleReportList();
        loadSubmissionPreview();
    });

    $reportSearchInput.on("input", function() {
        reportFilterText = $.trim($(this).val() || "").toLowerCase();
        renderEligibleReportList();
    });

    $reportClearSearchButton.on("click", function() {
        if (!reportFilterText) {
            return;
        }
    ...
    $reportClearSelectionButton.on("click", function() {
        if (selectedReportIds.length === 0) {
            return;
        }
    ...
    $rangeOptions.off("click").on("click", function() {
        submissionActionNote = "";
        clearTrackedBatch();
        setPresetTimeRange($(this));
        updateDraftSelection();
        loadSubmissionPreview();
    });

    $customRangeApplyButton.on("click", function() {
        submissionActionNote = "";
        clearTrackedBatch();
        if (!applyCustomTimeRange()) {
            updateDraftSelection();
            return;
        }
        updateDraftSelection();
    ...
    $customRangeEarliest.add($customRangeLatest).on("input", function() {
        $customRangeFeedback.removeClass("is-error");
        if (!$customRangeApplyButton.hasClass("is-active")) {
            $customRangeFeedback.text("Press Use Custom Range to apply the entered tokens.");
        }
    });

    $customRangeEarliest.add($customRangeLatest).on("keydown", function(event) {
        if (event.key === "Enter") {
            event.preventDefault();
            $customRangeApplyButton.trigger("click");
        }
    });

    $reviewAcknowledgement.off("change").on("change", function() {
        submissionActionNote = "";
        clearTrackedBatch();
        updateDraftSelection();
        loadSubmissionPreview();
    ...
    $recentBatchList.off("click", ".sutw-recent-batch-select").on("click", ".sutw-recent-batch-select", function() {
    ...
    $recentBatchRefreshButton.off("click").on("click", function() {
        loadRecentBatches({
            manual: true,
    ...
    resetSummaryState();
    $customRangeEarliest.val(defaultTimeRange.earliest);
    $customRangeLatest.val(defaultTimeRange.latest);
    setPresetTimeRange($rangeOptions.filter(".is-active").first());
